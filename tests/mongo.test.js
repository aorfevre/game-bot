...
describe("Mongdb tests", () => {
  ...
  test("Add 2 participations of the same user, pick participants ", async() => {
    ...
    await client.db("gaming").collection('tx').updateMany({verified:false},{$set:{verified:true}})
    await client.db("gaming").collection('tx').updateMany({tx:null},{$set:{tx:{hash:'123'}}})
    const txs = await helper.get_players_by_game_tiers("ROCKPAPERSCISSORS", "1");
    ...
    const txs2 = await helper.get_players_by_game_tiers("ROCKPAPERSCISSORS", "1");
